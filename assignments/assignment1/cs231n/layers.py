...
def svm_loss(x, y):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    correct_class_score = np.matrix(x[np.arange(x.shape[0]), y])
    margin = np.maximum(0, x- correct_class_score.T + 1)
    margin[np.arange(x.shape[0]), y] = 0
    loss = np.mean(np.sum(margin, axis=1))
    loss_count = np.sum(margin>0, axis=1)
    dx[margin>0] = 1
    dx[np.arange(x.shape[0]), y] = -loss_count.T
    dx /= x.shape[0]
